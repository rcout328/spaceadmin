...
export function AdminBestProperties() {
  ...
  async function fetchAllProperties() {
    try {
      const { data, error } = await supabase
        .from('properties')
        .select('*')
        .eq('top3', false); // Fetch properties that are not top3
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-6">Manage Best Properties</h1>
      <div className="bg-white shadow-md rounded-lg p-6 mb-8">
        <h2 className="text-2xl font-semibold mb-4">Current Top 3 Properties</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {bestProperties.map((property) => (
            <div key={property.id} className="border rounded-lg shadow-sm overflow-hidden">
              <img src={property.image} alt={property.property_name} className="w-full h-48 object-cover" />
              <div className="p-4">
                <h3 className="text-xl font-semibold mb-2">{property.property_name}</h3>
                <p className="text-gray-600 mb-4"><Home className="inline mr-1" size={16} />{property.city}</p>
                <Button
                  onClick={() => deleteProperty(property.id)}
                  className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md flex items-center justify-center w-full"
                >
                  <Trash2 className="mr-2" size={16} /> Remove from Top 3
                </Button>
              </div>
            </div>
          ))}
      ...
      {bestProperties.length < 3 && (
        <div className="bg-white shadow-md rounded-lg p-6">
          <h2 className="text-2xl font-semibold mb-4">Add Property to Top 3</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div key={property.id} className="border rounded-lg shadow-sm overflow-hidden">
                ...
                <div className="p-4">
                  ...
                  <Button
                    ...
                    className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-md flex items-center justify-center w-full"
                  >
                    <Plus className="mr-2" size={16} /> Add to Top 3
                  </Button>
                </div>
              </div>
            ))}
